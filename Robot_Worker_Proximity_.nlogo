...
indis-own [
  robot?         ;; BOOLEAN if agent is a manager
  ...
  myrbt            ;; my manager
  myrbtsmflr?      ;; BOOLEAN if agent's manager is on same floor
  crowdistrbt     ;; TO IMPLEMENT - determine point-to-point distance between manager and subordinate
  pathdistrbt      ;; TO IMPLEMENT
  timewrbt         ;; actually time spent colocated with manager (in seconds)
...
to setup-employees
  ...
  [ sprout 1
    [
      ...
      ;; set color blue
      set robot? FALSE
      if (rbt-same-floor-only) [  ;; if parameter set to same floor manager only, link with the manager on my floor
        ifelse xcor < 0
          [ create-link-with one-of indis with [ (robot? = TRUE) and (xcor < 0) ] ]
          [ create-link-with one-of indis with [ ( robot? = TRUE) and (xcor > 0) ] ]
      ]
      if (not rbt-same-floor-only) [  ;; otherwise, link with either manager
;        let rand random-float 1
;        ifelse (rand <= 0.5) [ create-link-with mgr1 ]
;                             [ create-link-with mgr2 ]
        create-link-with one-of indis with [ robot? = TRUE ]
      ]
      set myrbt one-of link-neighbors
      ask links [ hide-link ]
...
to set-instance-vars
  ...
  if (robot? = FALSE) [
  ifELSE ( [flr] of myrbt = [flr] of self ) [
    set myrbtsmflr? TRUE ]
  [ set myrbtsmflr? FALSE ]
  ]
...
to setup-agentsets
  ask rbt1 [
    set rbt1hasntseen link-neighbors
  ]

  ask rbt2 [
    set rbt2hasntseen link-neighbors
  ]

  ask rbt1 [
    set rbt1flr1emps link-neighbors with [ flr = 1 ]
    set rbt1flr2emps link-neighbors with [ flr = 2 ]
  ]

  ask rbt2 [
    set rbt2flr1emps link-neighbors with [ flr = 1 ]
    set rbt2flr2emps link-neighbors with [ flr = 2 ]
  ]
end
  ...
  ask indis [
    if any? other indis-here [
      if member? one-of link-neighbors indis-here [  ;; if my boss is one of the agents here
        if robot? = FALSE [
          set timewrbt timewrbt + 1             ;;   increment my "time with" counter
        ]
      ]
      record-encounter
    ]
...
to record-encounter
  ...
  if (any? tempagentset with [myrbt = myself])  [
      ask tempagentset with [ myrbt = myself ] [
        if ( [flr] of myrbt = 1) [
         set rbt1hasntseen other rbt1hasntseen ]
        if ( [flr] of myrbt = 2) [
         set rbt2hasntseen other rbt2hasntseen ]
       ]
  ]
...
draw-what?
draw-what?
"wall" "desk" "kitchen" "bathroom" "printer" "meeting room" "elevator" "employee" "robot" "nothing(clear)"
1
...
"" ""
PENS
"rbt1_flr1_emps" 1.0 0 -13345367 true "" "if (rbt1flr1emps != 0) [\nplot sum [ timewrbt ] of rbt1flr1emps\n]"
"rbt1_flr2_emps" 1.0 0 -11221820 true "" "if (rbt1flr2emps != 0) [\nplot sum [ timewrbt ] of rbt1flr2emps\n]"
"rbt2_flr2_emps" 1.0 0 -8630108 true "" "if (rbt2flr2emps != 0) [\nplot sum [ timewrbt ] of rbt2flr2emps\n]"
"rbt2_flr1_emps" 1.0 0 -5825686 true "" "if (rbt2flr1emps != 0) [\nplot sum [ timewrbt ] of rbt2flr1emps\n]"
...
177
401
% haven't seen robot today
( precision (\n  count indis with [ (timewrbt = 0) and (robot? = FALSE) ] / \n  count indis with [ robot? = FALSE ]\n) 4 ) * 100
17
1
11
